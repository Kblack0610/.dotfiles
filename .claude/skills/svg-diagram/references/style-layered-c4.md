# Style: layered / C4 (fleet/deck theme)

> One of two house styles in this skill. Use this **layered / C4 architecture** style
> (navy/coral/teal, transparent canvas, header-barred layer bands) for decks and
> altitude/architecture views. For process/data-flow with swimlanes (blue lanes + amber
> accent, white-PNG for Confluence) see `style-process-flow.md`. Exemplar:
> `assets/example-architecture-layers.svg`. Shared rendering/publish tooling:
> `rendering.md`. (Moved here from the marp-slide skill; marp now references this skill.)
