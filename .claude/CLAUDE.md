...
- `lab-sync` — refresh the **lab project bus** (`~/.notes/lab/projects/current/{name}/summary.md`): the human↔agent release/status/comms layer between `~/.agent` runtime and the in-repo CHANGELOG. Mirrors git + GitHub + Vikunja into the `## ← Release & status feed` AUTO **cockpit** (deterministic; `/lab-sync` + weekly `agentctl@lab-sync`) — shipped tag, Shipping-next (git log since tag), TRACKER sub-block (next-release ticket + In-Development tickets, preserved headless), In-flight PRs, Recent commits, drill-down links. Per-project `<!-- cockpit: k=v -->` marker. Your `## → For the agents` wants are injected at turn 1 by the preflight. Triggers: manual, weekly, on-release (release/bug-bash skills), on `/wind-down`
- `lab-status` — the **LLM half of the lab cockpit** (pairs with deterministic `lab-sync`): writes the dated "where we are" **status** narrative into the `STATUS` block, and **scopes** the human's wants under `## → For the agents` into tracked Vikunja tickets (via the `ticket` CLI; confirms before writing). Verbs: `status` | `scope`. Deterministic splicer: `skills/lab-sync/write-lab-status.sh`. Triggers: manual `/lab-status [project]`, on `/wind-down`. Opt-in LLM — kept out of the weekly cron.
