...
## Writing Style (all artifacts: docs, commits, tickets, ClickUp/Drive mirrors)

Mirrors the shared rule in `rulesync-global/.rulesync/rules/overview.md` (Claude reads this file, not the rulesync fan-out). Applies to every agent and subagent, including anything mirrored into ClickUp or Google Docs.

- Plain ASCII only. No fancy Unicode symbols: no em dash or en dash (use a regular hyphen, comma, or colon), no section sign (write "Section"), no arrows (write "->"), no >=, <=, x-times, middot, ellipsis, subscripts (write "T0"), or emoji. These are the tells that make a doc read as machine-written and they render badly in some viewers. Sweep every doc before saving.
- Do not invent label schemes (for example "F1..F27" IDs). Refer to things by the source document's own names and numbering.
- Do not hard-wrap prose or bullets to a column. Write each paragraph, bullet, and blockquote line as one physical line so renderers soft-wrap. Tables stay one row per line.
...
| Project release/status bus + human↔agent comms | `~/.notes/lab/projects/current/{name}/summary.md` (personal) or `~/.notes/employment/jobs/gigantic_playground/projects/{name}/summary.md` (Gigantic work); two-region: `## → For the agents` + `## ← Release & status feed`. TRACKER dispatched per-repo: Vikunja (personal) / ClickUp (Gigantic) | The slow, durable, multi-device BUS between `~/.agent` runtime and in-repo CHANGELOG. Agents post the feed via the **`lab-sync`** skill (`/lab-sync` + weekly `agentctl@lab-sync`, deterministic); you post comments/tasks under `## → For the agents` and the SessionStart preflight injects them at turn 1. `<!-- canonical: NAME -->` maps lab name → agent/repo name. |
...
- `meeting-ingest` — Krisp -> notes-vault pipeline: pull a meeting's transcript/summary/action-items from the Krisp MCP, fix Krisp's speaker mislabeling (it tags you as "Speaker_2"), write a profile-aware `notes meeting` entry (gigantic vault on the Mac / personal journal on Linux), then propose action items as tickets (propose->confirm, never auto). Verbs: `ingest` (one/recent) - `backfill` (bulk, notes-only). Deterministic writer: `skills/meeting-ingest/krisp-ingest.sh` (dedup ledger at `~/.local/state/meeting-ingest/`). Post-meeting automation scaffolded/disabled: `skills/meeting-ingest/AUTOMATION.md` + `.config/agentctl/agents/meeting-ingest.conf.disabled`
...
- `lab-sync` — refresh the **lab project bus** (personal: `~/.notes/lab/projects/current/{name}/summary.md`; Gigantic work: `~/.notes/employment/jobs/gigantic_playground/projects/{name}/summary.md` — both roots via `shared-hooks/lab-roots.sh`): the human↔agent release/status/comms layer between `~/.agent` runtime and the in-repo CHANGELOG. Mirrors git + GitHub + the repo's **tracker** into the `## ← Release & status feed` AUTO **cockpit** (deterministic; `/lab-sync` + weekly `agentctl@lab-sync`) — the TRACKER sub-block is dispatched per-repo by `project-map.json` `trackers.<canon>.system`: **Vikunja** (default/personal) or **ClickUp** (Gigantic). Rows: shipped tag, Shipping-next (git log since tag), TRACKER (In-Development tickets ± next-release ticket, preserved headless), In-flight PRs, Recent commits, drill-down links. Per-project `<!-- cockpit: k=v -->` marker. Your `## → For the agents` wants are injected at turn 1 by the preflight. Triggers: manual, weekly, on-release (release/bug-bash skills), on `/wind-down`
- `lab-status` — the **LLM half of the lab cockpit** (pairs with deterministic `lab-sync`): writes the dated "where we are" **status** narrative into the `STATUS` block, and **scopes** the human's wants under `## → For the agents` into tracked tickets — Vikunja or ClickUp, whichever the repo's tracker is (via the tracker-agnostic `ticket` CLI; confirms before writing). Verbs: `status` | `scope`. Deterministic splicer: `skills/lab-sync/write-lab-status.sh`. Triggers: manual `/lab-status [project]`, on `/wind-down`. Opt-in LLM — kept out of the weekly cron.
...
- `ingest-worktree` — land a pile of hanging working-tree changes correctly: survey the dirty tree, group every change by concern, write conventional-commit messages, and ship each concern as its own branch + PR (never one mixed blob). HOLDS machine-local/ephemeral churn (theme-switch output) and scans untracked files for secrets before they go public. Branch-first off the default branch; confirms before the first push. Differs from `sc:git` (single commit) and `my:pr-merge-flow` (drives one existing PR) — this fans a dirty tree OUT into correctly-scoped PRs
