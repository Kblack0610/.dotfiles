---
description: Headless coordinator for the kb-* pipeline. Runs brief → spec → plan-check → code → review
  → qa end-to-end and returns a structured JSON result. Use when there is no human in the loop (CI, cron,
  claude -p). For interactive sessions prefer the /kb:workflow skill.
mode: subagent
---
