...
- **Windows-side native tools** via winget: git, neovim, ripgrep, fd, fzf, lazygit, starship, gsudo. All on `$PATH` and usable directly in PowerShell.
...
2. `winget install`s these IDs (each step is skipped if the package is already present):
   ...
   - `Starship.Starship`, `gerardog.gsudo`
