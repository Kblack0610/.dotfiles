# Package Manager Configuration for Debian/Ubuntu
PACKAGE_MANAGER="apt"
...
function install_reqs() {
	...
	#Base requirements
	yes | $PACKAGE_UPDATE_CMD &> /dev/null 

  #not necessary but reqs for my tools
	yes | $PACKAGE_INSTALL_CMD vim &> /dev/null
	yes | $PACKAGE_INSTALL_CMD wget &> /dev/null
	yes | $PACKAGE_INSTALL_CMD curl &> /dev/null

  yes | $PACKAGE_INSTALL_CMD libfuse2 &> /dev/null
	yes | $PACKAGE_INSTALL_CMD neofetch &> /dev/null

  #Screenshots
  yes | $PACKAGE_INSTALL_CMD maim &> /dev/null
